...
class Broyden
{
  private:
    ...
    void ludcmp(complex<double> **a, int n, int *indx);				//LU decomposes **a
    void lubksb(complex<double> **a, int n, int *indx, complex<double> b[]);	//backtrace method
    void InverseMatrix(complex<double> **a, complex<double> **y, int n);	//returns inverse of a (1..n x 1..n) in y  
  ...
  public:
    ...
    void TurnOff();
    void Reset(int it);
    bool CalculateNew(complex<double> Vnew[], int it);
    double CurrentDiff;
};
                ...
                void (T::*func)(complex<double>*), T* obj, complex<double>* V)
{
  ...
  { printf("    Broyden: Iteration %d...\n", it);
    ...
    if (B.CalculateNew(V, it)) { b=true; break; } 
    ...
    if (( (B.CurrentDiff > 100.0 * OldDiff) or (B.CurrentDiff > 10) ) and (it>8))
